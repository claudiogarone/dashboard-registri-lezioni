...
FOLDER_ID = "1fsy7Ep3Kbyfhx3NEfnZDryHNfefHmDBM"  # ID cartella "Registri Lezioni"
...
def load_all_data(_creds, files):
    ...
    if "Allievo" not in data.columns or data["Allievo"].isna().all():
        data["Allievo"] = data["__file_name"]

    if "Data" in data.columns:
        data["Data_dt"] = pd.to_datetime(data["Data"], errors="coerce", dayfirst=True)
    ...
    pay_col = None
    for cand in ["Pagamento Ricevuto (EUR)", "Pagamento Ricevuto (\u20ac)"]:
        if cand in data.columns:
            pay_col = cand
            break
    if pay_col:
        data["Pagamento_num"] = pd.to_numeric(
            data[pay_col].astype(str).str.replace(",", ".", regex=False),
            errors="coerce",
        ).fillna(0)
    else:
        data["Pagamento_num"] = 0

    if "Data Pagamento" in data.columns:
        data["Data_Pagamento_dt"] = pd.to_datetime(
            data["Data Pagamento"], errors="coerce", dayfirst=True
        )

    if "Pagamento Ricevuto" in data.columns:
        data["Stato_Pagamento"] = data["Pagamento Ricevuto"].fillna("Non specificato").replace("", "Non specificato")
    else:
...
allievi = sorted(df["Allievo"].dropna().unique().tolist())
sel_allievi = st.sidebar.multiselect("Allievo", allievi, default=allievi)

materie = sorted(df["Materia"].dropna().unique().tolist()) if "Materia" in df.columns else []
...
stati_pagamento = sorted(df["Stato_Pagamento"].dropna().unique().tolist())
sel_stati_pagamento = st.sidebar.multiselect("Stato pagamento", stati_pagamento, default=stati_pagamento)

compito_assegnato_opts = sorted(df["Compito Assegnato (Si/No)"].dropna().unique().tolist()) if "Compito Assegnato (Si/No)" in df.columns else []
...
compito_superato_opts = sorted(df["Compito Superato (Si/No)"].dropna().unique().tolist()) if "Compito Superato (Si/No)" in df.columns else []
...
if materie:
    mask &= df["Materia"].isin(sel_materie)
if isinstance(date_range, tuple) and len(date_range) == 2:
    start, end = date_range
    mask &= df["Data_dt"].dt.date.between(start, end)
if sel_stati_pagamento:
    mask &= df["Stato_Pagamento"].isin(sel_stati_pagamento)
if compito_assegnato_opts:
    mask &= df["Compito Assegnato (Si/No)"].isin(sel_compito_assegnato)
if compito_superato_opts:
    mask &= df["Compito Superato (Si/No)"].isin(sel_compito_superato)
if mesi_disponibili:
    mask &= df["Mese"].isin(sel_mesi)
if anni_disponibili:
    mask &= df["Anno"].isin(sel_anni)
if argomento_search:
    cols_txt = [c for c in ["Argomento", "Note"] if c in df.columns]
    if cols_txt:
        txt_mask = pd.Series(False, index=df.index)
        for c in cols_txt:
            txt_mask |= df[c].astype(str).str.contains(argomento_search, case=False, na=False)
        mask &= txt_mask
...
col3.metric("Allievi", fdf["Allievo"].nunique())
col4.metric("Incassato", money_fmt(fdf["Pagamento_num"].sum()))
non_pagate = fdf[fdf["Stato_Pagamento"].astype(str).str.lower().str.contains("no", na=False)]
...
with tab_argomenti:
    ...
    if "Argomento" in fdf.columns:
        ...
        st.dataframe(
            arg_df[[c for c in ["Data", "Allievo", "Materia", "Numero Lezione", "Argomento"] if c in arg_df.columns]]
            .sort_values("Data_dt" if "Data_dt" in arg_df else "Data"),
            use_container_width=True, hide_index=True,
        )
...
with tab_presenze:
    ...
    st.caption("Proxy: ogni riga registrata equivale a una lezione svolta (non e' presente una colonna assenze dedicata).")
...
with tab_dettaglio:
    ...
    st.dataframe(fdf.drop(columns=["Data_dt", "Data_Pagamento_dt"], errors="ignore"),
                 use_container_width=True, hide_index=True)
    csv = fdf.drop(columns=["Data_dt", "Data_Pagamento_dt"], errors="ignore").to_csv(index=False).encode("utf-8")
