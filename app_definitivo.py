...
def periodize(dt_series, granularita):
...
@st.cache_data(ttl=60, show_spinner="Aggiornamento elenco fogli...")
...
@st.cache_data(ttl=60, show_spinner="Caricamento registri lezioni...")
def load_all_data(_creds, files):
    ...
    if "Data" in data.columns:
        data["Data_dt"] = pd.to_datetime(
            data["Data"].astype(str).str.strip(), errors="coerce", dayfirst=True
        )
...
col2.metric("Ore totali", round(fdf["Totale Ore_h"].sum(), 1) if "Totale Ore_h" in fdf.columns else "-")
...
with tab_ore:
    ...
    if "Data_dt" in fdf.columns and "Totale Ore_h" in fdf.columns:
        trend = fdf.dropna(subset=["Data_dt"]).copy()
        if not trend.empty:
            trend["Periodo"] = periodize(trend["Data_dt"], granularita)
            trend = trend.groupby(["Periodo", "Allievo"])["Totale Ore_h"].sum().reset_index()
            if not trend.empty:
                fig1 = px.line(trend, x="Periodo", y="Totale Ore_h", color="Allievo", markers=True,
                               labels={"Periodo": granularita, "Totale Ore_h": "Ore"})
...
with tab_soldi:
    ...
    if "Data_dt" in fdf.columns:
        pay_time = fdf.dropna(subset=["Data_dt"]).copy()
        if not pay_time.empty:
            pay_time["Periodo"] = periodize(pay_time["Data_dt"], gran_pay)
            pay_time = pay_time.groupby(["Periodo"])["Pagamento_num"].sum().reset_index()
            pay_time["Cumulato"] = pay_time["Pagamento_num"].cumsum()
            c1, c2 = st.columns(2)
            with c1:
                if not pay_time.empty:
                    fig_pt = px.bar(pay_time, x="Periodo", y="Pagamento_num",
                                    labels={"Periodo": gran_pay, "Pagamento_num": "Incassato (EUR)"},
                                    title="Incassato per periodo")
                    st.plotly_chart(fig_pt, use_container_width=True)
            with c2:
                if not pay_time.empty:
                    fig_cum = px.area(pay_time, x="Periodo", y="Cumulato",
                                      labels={"Periodo": gran_pay, "Cumulato": "Incassato cumulato (EUR)"},
...
with tab_presenze:
    ...
    if "Voto" in fdf.columns:
        ...
        if not voto_df.empty and "Data_dt" in voto_df.columns:
